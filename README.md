...
PSM) through the Spark ALM Controller and ALMProxy.

The goal is resilience: withdrawals do not depend on the ALM Planner being online. Any user above
the 24/7 liquidity level can trigger a withdrawal for the major supported assets (USDC, USDT, and WETH) without an
...
The diagram below shows the end to end withdrawal flow for both implementations: the contract sources any
shortfall from the venue into the vault through the Controller and ALMProxy, the user redeems shares
from the vault, and the proceeds are split between the recipient and the penalty recipient.
