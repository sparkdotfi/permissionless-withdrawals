...
import {
    ForkTestBase,
    IERC20,
    IMainnetControllerLike,
    ISparkVaultLike
} from "./ForkTestBase.t.sol";

contract PermissionlessWithdrawalsTest is ForkTestBase {
    ...
    function test_permissionlessWithdraw_notRelayer() external {
        ...
        deal(address(WETH), address(mainnetController.proxy()), 0);

        // Revoke the relayer role from the withdrawals contract
        vm.prank(freezer);
        mainnetController.removeRelayer(address(withdrawals));
    ...
    function test_permissionlessWithdraw_insufficientVenueLiquidity() external {
        ...
        // Mock the withdrawAave call to return 0 assets
        vm.mockCall(
            address(mainnetController),
            abi.encodeCall(IMainnetControllerLike.withdrawAave, (SparkLend.WETH_SPTOKEN, assets)),
            abi.encode(0)
        );

        vm.expectRevert(abi.encodeWithSelector(IPermissionlessWithdrawals.InsufficientVenueLiquidity.selector, assets, 0));
        vm.prank(user);
        withdrawals.permissionlessWithdraw(address(spETHVault), SparkLend.WETH_SPTOKEN, recipient, shares);
    }
    ...
    function test_permissionlessWithdraw_vaultCoversFullAmount_spETH() external {
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(WETH), address(spETHVault), assets)), 0);
    ...
    function test_permissionlessWithdraw_vaultCoversFullAmount_spUSDC() external {
        ...
        // No transferAsset call is expected, since the vault alone covers the full amount.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(USDC), address(spUSDCVault), assets)), 0);
    ...
    function test_permissionlessWithdraw_vaultCoversFullAmount_spUSDT() external {
        ...
        // No transferAsset call is expected, since the vault alone covers the full amount.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(USDT), address(spUSDTVault), assets)), 0);
    ...
    function test_permissionlessWithdraw_proxyCoversFullAmount_spETH() external {
        ...
        deal(address(WETH), address(mainnetController.proxy()), assets);
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,    (SparkLend.WETH_SPTOKEN, assets)),                    0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (SparkLend.WETH_SPTOKEN, assets, type(uint256).max)), 0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC,  (assets)),                                            0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(WETH), address(spETHVault), assets)),        1);
        vm.expectCall(address(spETHVault),        abi.encodeCall(ISparkVaultLike.redeem,                 (shares, address(withdrawals), user)),                1);
    ...
    function test_permissionlessWithdraw_proxyCoversFullAmount_spUSDC() external {
        ...
        // Dealing assets to the proxy to cover the full amount.
        deal(address(USDC), address(mainnetController.proxy()), assets);
        ...
        // No venue withdrawal is needed, since the proxy covers the full amount.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,    (Ethereum.PSM, assets)),                        0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (Ethereum.PSM, assets, type(uint256).max)),     0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC,  (assets)),                                      0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(USDC), address(spUSDCVault), assets)), 1);
        vm.expectCall(address(spUSDCVault),       abi.encodeCall(ISparkVaultLike.redeem,                 (shares, address(withdrawals), user)),          1);
    ...
    function test_permissionlessWithdraw_proxyCoversFullAmount_spUSDT() external {
        ...
        // No venue withdrawal is needed, since the proxy covers the full amount.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,    (SparkLend.USDT_SPTOKEN, assets)),                    0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (SparkLend.USDT_SPTOKEN, assets, type(uint256).max)), 0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC,  (assets)),                                            0);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(USDT), address(spUSDTVault), assets)),       1);
        vm.expectCall(address(spUSDTVault),       abi.encodeCall(ISparkVaultLike.redeem,                 (shares, address(withdrawals), user)),                1);
    ...
    function test_permissionlessWithdraw_aaveVenueCoversFullAmount_spETH() external {
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,  (SparkLend.WETH_SPTOKEN, assets)),             1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(WETH), address(spETHVault), assets)), 1);
        vm.expectCall(address(spETHVault),        abi.encodeCall(ISparkVaultLike.redeem,               (shares, address(withdrawals), user)),         1);
    ...
    function test_permissionlessWithdraw_aaveVenueCoversFullAmount_spUSDT() external {
        ...
        // Proxy holds no idle, so the full amount must be withdrawn from the Aave venue.
        deal(address(USDT), address(mainnetController.proxy()), 0);
        ...
        // The full amount is withdrawn from the Aave venue and transferred to the vault.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,  (SparkLend.USDT_SPTOKEN, assets)),              1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(USDT), address(spUSDTVault), assets)), 1);
        vm.expectCall(address(spUSDTVault),       abi.encodeCall(ISparkVaultLike.redeem,               (shares, address(withdrawals), user)),          1);
    ...
    function test_permissionlessWithdraw_erc4626VenueCoversFullAmount_spUSDC() external {
        ...
        vm.prank(admin);
        withdrawals.updateVenueConfig(Ethereum.MORPHO_VAULT_USDC_BC, IPermissionlessWithdrawals.VenueType.ERC4626, true);
        ...
        deal(address(USDC), address(mainnetController.proxy()), 0);
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (Ethereum.MORPHO_VAULT_USDC_BC, assets, type(uint256).max)), 1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(USDC), address(spUSDCVault), assets)),               1);
        vm.expectCall(address(spUSDCVault),       abi.encodeCall(ISparkVaultLike.redeem,                 (shares, address(withdrawals), user)),                       1);
    ...
    function test_permissionlessWithdraw_psmVenueCoversFullAmount_spUSDC() external {
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.mintUSDS,       (assets * 1e12)),                               1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC, (assets)),                                      1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,  (address(USDC), address(spUSDCVault), assets)), 1);
        vm.expectCall(address(spUSDCVault),       abi.encodeCall(ISparkVaultLike.redeem,                (shares, address(withdrawals), user)),          1);
    ...
    function test_permissionlessWithdraw_vaultProxyAaveVenueCoversFullAmount_spETH() external {
        ...
        deal(address(WETH), address(mainnetController.proxy()), proxyAmount);
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,  (SparkLend.WETH_SPTOKEN, assetsToWithdraw)),             1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(WETH), address(spETHVault), assetsToTransfer)), 1);
        vm.expectCall(address(spETHVault),        abi.encodeCall(ISparkVaultLike.redeem,               (shares, address(withdrawals), user)),                   1);
    ...
    function test_permissionlessWithdraw_vaultProxyAaveVenueCoversFullAmount_spUSDT() external {
        ...
        // The proxy holds part of the shortfall, so the venue must cover the remainder.
        deal(address(USDT), address(mainnetController.proxy()), proxyAmount);
        ...
        // The venue covers only the remaining shortfall, then the full shortfall is transferred to the vault.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,  (SparkLend.USDT_SPTOKEN, assetsToWithdraw)),              1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset, (address(USDT), address(spUSDTVault), assetsToTransfer)), 1);
        vm.expectCall(address(spUSDTVault),       abi.encodeCall(ISparkVaultLike.redeem,               (shares, address(withdrawals), user)),                    1);
    ...
    function test_permissionlessWithdraw_vaultProxyERC4626VenueCoversFullAmount_spUSDC() external {
        ...
        // The proxy holds part of the shortfall, so the venue must cover the remainder.
        deal(address(USDC), address(mainnetController.proxy()), proxyAmount);
        ...
        // The venue covers only the remaining shortfall, then the full shortfall is transferred to the vault.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (Ethereum.MORPHO_VAULT_USDC_BC, assetsToWithdraw, type(uint256).max)), 1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(USDC), address(spUSDCVault), assetsToTransfer)),              1);
        vm.expectCall(address(spUSDCVault),       abi.encodeCall(ISparkVaultLike.redeem,                 (shares, address(withdrawals), user)),                                 1);
    ...
    function test_permissionlessWithdraw_vaultProxyPsmVenueCoversFullAmount_spUSDC() external {
        ...
        // The venue covers only the remaining shortfall, then the full shortfall is transferred to the vault.
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.mintUSDS,       (assetsToWithdraw * 1e12)),                               1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC, (assetsToWithdraw)),                                      1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,  (address(USDC), address(spUSDCVault), assetsToTransfer)), 1);
        vm.expectCall(address(spUSDCVault),       abi.encodeCall(ISparkVaultLike.redeem,                (shares, address(withdrawals), user)),                    1);
    ...
    function test_permissionlessWithdraw_multipleWithdrawals_spUSDC() external {
        ...
        vm.startPrank(admin);
        withdrawals.updateVenueConfig(Ethereum.MORPHO_VAULT_USDC_BC, IPermissionlessWithdrawals.VenueType.ERC4626, true);
        withdrawals.updateVenueConfig(SparkLend.USDC_SPTOKEN,        IPermissionlessWithdrawals.VenueType.AAVE,    true);
        vm.stopPrank();
        ...
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawERC4626, (Ethereum.MORPHO_VAULT_USDC_BC, assetsEach, type(uint256).max)), 1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.withdrawAave,    (SparkLend.USDC_SPTOKEN, assetsEach)),                           1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.mintUSDS,        (assetsEach * 1e12)),                                            1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.swapUSDSToUSDC,  (assetsEach)),                                                   1);
        vm.expectCall(address(mainnetController), abi.encodeCall(IMainnetControllerLike.transferAsset,   (address(USDC), address(spUSDCVault), assetsEach)),              3);
