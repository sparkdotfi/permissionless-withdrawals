...
1. Validate that `vault` and `recipient` are non-zero, `shares` is non-zero, the controller/proxy consistency check passes, the contract is an authorized allocator/relayer, and that the vault is whitelisted.
...
A vault can be disabled (unwhitelisted) via `setVaultConfig` despite maintaining valid (non-zero) configurations for its penalty amount and any venue type mappings set via `setVenueType`. Because these parameters remain in storage, re-enabling the vault in the future will automatically use the previously set configuration, unless the admin explicitly clears or deletes them manually.
...
- The admin must dewhitelist the vault via `setVaultConfig`.
